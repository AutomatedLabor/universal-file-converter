...
pub struct OpusPlugin;
impl OpusPlugin { pub fn new() -> Self { Self } }

impl ConverterPlugin for OpusPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            id: "core-opus".to_string(), version: semver::Version::new(1, 0, 0),
            api_version: semver::Version::new(1, 0, 0), author: "UFC Core Team".to_string(),
            license: "MIT".to_string(), description: "Opus audio decoder".to_string(),
            input_formats: vec![FormatId::new("audio/opus", &["opus"], "Opus")],
            output_formats: vec![],
    ...
    fn probe(&self, input: &FileReader) -> Result<ProbeResult, PluginError> {
        ...
        if header.len() >= 4 && &header[0..4] == b"OggS" {
            // Could be Opus in OGG container
            let ext = input.path().extension().map(|e| e.to_string_lossy().to_lowercase());
            if ext.as_deref() == Some("opus") {
                return Ok(ProbeResult { confidence: 95, detected_format: FormatId::new("audio/opus", &["opus"], "Opus"),
                    format_version: None, estimated_size: Some(input.size()), warnings: vec![] });
            }
        }
        Err(PluginError::UnsupportedFormat("Not an Opus file".to_string()))
    }
    ...
    fn decode(&self, input: &FileReader, _config: &DecodeConfig, progress: &ProgressCallback) -> Result<Box<dyn Any + Send + Sync>, PluginError> {
        ...
        let hint = symphonia::core::probe::Hint::new().with_extension("opus");
        ...
        let mut probed = symphonia::default::get_probe()
            ...
            .map_err(|e| PluginError::DecodeFailed(format!("Opus probe: {}", e)))?;
        ...
        let mut decoder = codecs.make(&codec_params, &Default::default())
            .map_err(|e| PluginError::DecodeFailed(format!("Opus decoder: {}", e)))?;
        ...
        ir.metadata.original_format = "Opus".to_string();
    ...
    fn encode(&self, _ir: &(dyn Any + Send + Sync), _output: &FileWriter, _config: &EncodeConfig, _progress: &ProgressCallback) -> Result<ConversionOutput, PluginError> {
        Err(PluginError::EncodeFailed("Opus encoding not supported".to_string()))
    }
