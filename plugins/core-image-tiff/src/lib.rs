...
pub struct TiffPlugin;
impl TiffPlugin { pub fn new() -> Self { Self } }

impl ConverterPlugin for TiffPlugin {
    fn manifest(&self) -> PluginManifest {
        PluginManifest {
            id: "core-tiff".to_string(), version: semver::Version::new(1, 0, 0),
            api_version: semver::Version::new(1, 0, 0), author: "UFC Core Team".to_string(),
            license: "MIT".to_string(), description: "TIFF image decoder and encoder".to_string(),
            input_formats: vec![FormatId::new("image/tiff", &["tiff", "tif"], "TIFF")],
            output_formats: vec![FormatId::new("image/tiff", &["tiff", "tif"], "TIFF")],
            capabilities: Capabilities { metadata: MetadataSupport::ReadWrite, structure: StructureSupport::Flat,
                embedded_assets: EmbeddedAssetSupport::None, color_spaces: vec![ColorSpace::Rgb, ColorSpace::Rgba, ColorSpace::Cmyk, ColorSpace::Gray],
                max_dimension: None, max_bit_depth: Some(16), supports_animation: false,
                supports_transparency: true, supports_multi_page: true },
            dependencies: vec![], priority: 80, fidelity_score: 100, known_limitations: vec![],
    ...
    fn probe(&self, input: &FileReader) -> Result<ProbeResult, PluginError> {
        ...
        if header.len() >= 4 && ((header[0] == b'I' && header[1] == b'I') || (header[0] == b'M' && header[1] == b'M')) {
            Ok(ProbeResult { confidence: 100, detected_format: FormatId::new("image/tiff", &["tiff", "tif"], "TIFF"),
                format_version: None, estimated_size: Some(input.size()), warnings: vec![] })
        } else { Err(PluginError::UnsupportedFormat("Not a TIFF file".to_string())) }
    }
    ...
    fn decode(&self, input: &FileReader, _config: &DecodeConfig, progress: &ProgressCallback) -> Result<Box<dyn Any + Send + Sync>, PluginError> {
        ...
        let img = image::load_from_memory(&data).map_err(|e| PluginError::DecodeFailed(format!("TIFF decode: {}", e)))?;
        let rgba = img.to_rgba8(); let (w, h) = rgba.dimensions();
        let mut ir = ImageIR::new(w, h, ColorSpace::Rgba, BitDepth::U8);
        ir.pixels = PixelData::Raw(rgba.into_raw()); ir.metadata.format_name = "TIFF".to_string();
    ...
    fn encode(&self, ir: &(dyn Any + Send + Sync), output: &FileWriter, _config: &EncodeConfig, progress: &ProgressCallback) -> Result<ConversionOutput, PluginError> {
        ...
        { use std::io::Cursor; let mut c = Cursor::new(&mut tiff_data);
          img.write_to(&mut c, image::ImageFormat::Tiff).map_err(|e| PluginError::EncodeFailed(format!("TIFF encode: {}", e)))?; }
        output.write_all(&tiff_data).map_err(|e| PluginError::IoError(e.to_string()))?;
        let checksum = blake3::hash(&tiff_data).to_hex().to_string();
        Ok(ConversionOutput { bytes_written: tiff_data.len() as u64, checksum, warnings: vec![], fidelity_estimate: 100 })
    }
